...
const TableCompanyComponent = memo(() => {
    ...
    async function loadData() {
        ...
        await apiService.obtenerCompañias().then(async resp => {
            resp = resp.result;
            setdataCompany(resp)
            drawRows(Paginaton(resp, 1, 5));

        })
    }
    function drawRows(resp) {
         apiService.getDriver().then(async conductor => {
            conductor = conductor.result;
            setdataConductor(() => { return conductor })
                ...
                <React.Fragment>
                    <tr key={uniq_key()}>
                        ...
                        <td style={{ textAlign: "center" }}>
                            ...
                            {conductor && conductor.find(x => x.company_id == data.id) ? null :
    ...
    return (
        <React.Fragment>
            <LoadingOverlay active={activeLoading} spinner text="Procesando información, espere por favor...">
                <Row>
                    <Col md="12" style={{ textAlign: "right", marginBottom: "15px" }} ><Button onClick={() => { setShow(true); settype("nuevo"); setdataSet(nuevoDatoCompany) }} variant="success">Nuevo</Button></Col>
                </Row>
                <Row>
                    <Col md="12">
                ...
                <Row>
                    <Col md="12" style={{ textAlign: "right" }}>
                        {dataCompany && <PaginationComponent data={dataCompany} drawRows={drawRows} />}
                    </Col>
                </Row>
            </LoadingOverlay>
