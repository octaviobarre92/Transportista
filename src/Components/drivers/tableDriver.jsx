  ...
  async function loadData() {
    ...
    await apiService.obtenerCompañias().then(async company => {
      ...
      await apiService.getDriver().then(async resp => {
        ...
        drawRows(Paginaton(resp, 1, 5), company)
      })
    })
  }
  function drawRows(resp, company = dataCompany) {
    apiService.getVehicle().then(async conductor => {
      ...
      <LoadingOverlay active={activeLoading} spinner text="Procesando información, espere por favor...">
        ...
        <Row>
          <Col md="12" style={{ textAlign: "right" }}>
            {dataDriver && <PaginationComponent data={dataDriver} drawRows={drawRows} />}
          </Col>
        </Row>
      </LoadingOverlay>
