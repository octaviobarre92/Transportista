...
const ModalVehicleComponent = memo((props) => {
    const { type, saveData, editData, handleClose, show, dataSet, dataSetDriver, dataVehicle } = props;
                                ...
                                <Form.Group className="mb-3" controlId="driver_int">
                                    ...
                                    <Form.Select defaultValue={dataSet?.driver_int} required name="company_id" aria-label="Default select example">
                                        {dataSetDriver && dataSetDriver.map((data) => (
                                            dataVehicle && dataVehicle.filter(x => x.driver_int == data.id_driver).length < 3
                                                ? < option key={uniq_key()} value={data.id_driver} > {data.first_name + " " + data.last_name}</option> : null
                                        ))}
                                    </Form.Select>
                                </Form.Group>
                            </Col>
