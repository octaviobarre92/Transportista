...
const TableVehicleDriverComponent = memo((props) => {
    ...
    function saveData(data) {
        ...
        apiService.newVehicle(data).then(resp => {
            setactiveLoading(false)
            loadData()
            settoast(true)
            handleClose();
    ...
    function editData(data) {
        ...
        apiService.updateVehicle(data).then(resp => {
            setactiveLoading(false)
            loadData()
            settoast(true)
            handleClose();
    ...
    function deleteData(data) {
        apiService.deleteVehicle(data).then(resp => {
            setactiveLoading(false)
            loadData()
            settoast(true)
        }).catch(err => {
    ...
    async function consult(data = null) {
        if(data==null) return;
        setactiveLoading(true)
        await apiService.getVehicle().then(async resp => {
            resp = resp.result;
            resp = resp.filter(x => x.driver_int == data);
            setactiveLoading(false)
            setdataRows(resp.map((data, index) => (
                <React.Fragment>
                    <tr key={uniq_key()}>
                        ...
                        <td>{moment(data.creation_date, 'YYYY-MM-DD').format('YYYY-MM-DD')}</td>
                        <td style={{ textAlign: "center" }}>
                            <Button variant="dark" onClick={() => { handleShowCompany(); setdataSetDriverNew(dataSetDriver.find(x => x.id_driver == data.driver_int)); settype("edit") }}>{dataSetDriver.find(x => x.id_driver == data.driver_int)?.first_name ?? ""}{" " + dataSetDriver.find(x => x.id_driver == data.driver_int)?.last_name}</Button>
                        </td>
                        <td style={{ textAlign: "center" }}>
                            <Button variant="dark" onClick={() => {
                                handleShowCompanyNew(); setdataSetCompanyNew(dataSetCompany.find(x => x.id == dataSetDriver.find(x => x.id_driver == data.driver_int)?.company_id)); settype("edit")
                            }}>{dataSetCompany.find(x => x.id == dataSetDriver.find(x => x.id_driver == data.driver_int)?.company_id).name ?? ""}</Button>
                        </td>
                    </tr>
                </React.Fragment>
            )));
    ...
    return (
        <React.Fragment>
                    ...
                    <Col md="4">
                        <>
                            <Form.Group className="mb-3" controlId="driver_int">
                                ...
                                <Form.Select onChange={(e) => { consult(e.target.value) }} name="company_id" aria-label="Default select example">
                                    <option key={uniq_key()} value="0">Seleccionar...</option>
                                    {dataSetDriver && dataSetDriver.map((data) => (
                                        <option key={uniq_key()} value={data.id_driver}>{data.first_name + " " + data.last_name}</option>
                                    ))}
                                </Form.Select>
                            </Form.Group>
                            <Form.Text id="passwordHelpBlock" muted>
                                Selecciona aqui al conductor por el que deseas que se filtre la tabla
                            </Form.Text>
                        </>
                    </Col>
                </Row>
            ...
            {dataSet && <ModalVehicleComponent dataSetDriver={dataSetDriver} show={show} handleClose={handleClose} saveData={saveData} editData={editData} dataSet={dataSet} type={type} />}
        </React.Fragment >
    )
})
export default TableVehicleDriverComponent